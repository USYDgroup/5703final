...
def create_dataset(dataset, config):
    ...
    if dataset=='vqa':
        train_dataset = vqa_dataset(config['train_file'], train_transform,config['data_root'], split='train')
        vqa_test_dataset = vqa_dataset(config['test_file'], test_transform,config['data_root'], split='test', answer_list=config['answer_list'])
...
def vqa_val_collate_fn(batch):
    image_list,question_list,question_id_list,answer_list=[],[],[],[]
    for image,question,question_id,answer in batch:
        image_list.append(image)
        question_list.append(question)
        question_id_list.append(question_id)
        answer_list.append(answer)
    
    return torch.stack(image_list,dim=0),question_list,question_id_list,answer_list
