...
class PreVQA(nn.Module):
    def __init__(self,                 
        ...
        config_fusion_encoder = BertConfig.from_json_file(config['bert_config'])
        config_fusion_encoder.fusion_layer = 0
        config_fusion_encoder.num_hidden_layers = 6
        # config_fusion_encoder.max_position_embeddings=578
        self.fusion_encoder = BertModel.from_pretrained(text_encoder, config=config_fusion_encoder, add_pooling_layer=False)
    ...
    def forward(self, image, quesiton, answer=None, alpha=0, k=None, weights=None, train=True):
        ...
        new_image_embeds = image_embeds.clone().detach()
        new_image_atts = torch.ones(new_image_embeds.size()[:-1],dtype=torch.long).to(image.device)
        init_weights(self,self.inversion)
        img_inversion = self.inversion(image_embeds[:,-1,:]) #[b,1,768]
        prompt = self.tokenizer(self.prompt, padding='longest', truncation=True, max_length=25, return_tensors="pt").to(image.device)
        prompt_emb = self.text_encoder.embeddings(prompt.input_ids)
        prompt_emb = prompt_emb.repeat(img_inversion.shape[0],1,1) #[b,5,768]
        prompt_emb[:,4] = img_inversion 
        text_output = self.text_encoder(inputs_embeds=prompt_emb,mode="text",return_dict = True).last_hidden_state #[B,5,768]
        text_attns = torch.ones(text_output.size()[:-1],dtype=torch.long).to(image.device)
        fusion_output = self.fusion_encoder(inputs_embeds=text_output,
                                            
                                           encoder_hidden_states = new_image_embeds,
                                           encoder_attention_mask = new_image_atts
                                           ).last_hidden_state
        image_embeds = torch.cat([image_embeds,fusion_output],dim=1)
