...
from dataset import create_dataset, create_sampler, create_loader, vqa_collate_fn, vqa_val_collate_fn
...
def evaluation(model, data_loader, tokenizer, device, config) :
    ...
    for n, (image, question, question_id,ans) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        for ques_id, topk_id, topk_prob,answer in zip(question_id, topk_ids, topk_probs,ans):
            
            total += 1
            ques_id = int(ques_id)
            _, pred = topk_prob.max(dim=0)
            pred_text = data_loader.dataset.answer_list[topk_id[pred]]
            result.append({"question_id":ques_id, "answer":pred_text,"gt":answer})
            # print(answer,pred_text)
            if pred_text in answer:
                acc += 1
            metric_logger.update(acc=acc/total)
...
def main(args, config):
    ...
    train_loader, test_loader = create_loader(datasets,samplers,
                                            ...
                                            collate_fns=[vqa_collate_fn,vqa_val_collate_fn]) 
    ...
    if args.checkpoint and not args.evaluate:
        ...
        if not args.evaluate:
            ...
            #                 ##new 
            #                 if 'bert' in key:
            #                     fusion_encoder_key = encoder_key.replace('bert.','')
            #                 fusion_encoder_key = fusion_encoder_key.replace('text_encoder','fusion_encoder')
            #                 state_dict[fusion_encoder_key] = state_dict[key]
                            
            #         else:
            #             encoder_key = key
                        
            #             ##new
            #             if 'embeddings' in key and 'bert' in key:
            #                 fusion_encoder_key = key.replace('bert.','')
            #                 fusion_encoder_key = fusion_encoder_key.replace('text_encoder','fusion_encoder')
            #                 state_dict[fusion_encoder_key] = state_dict[key]
        ...
        #加载albef fusion层到fusion_encoder
